...
win_len = 192
step = 10
model = models.load_model('Amerilike_8ch_rem_2_6.h5')
...
def pre_processing(Labels, Tables, recs, FSs):
...
def main():
    ...
    Tables[0] = Tables[0].drop(["Labjack_channel" + str(rem[0]), "Labjack_channel" + str(rem[1])],
                               axis='columns')
    Tables[0].columns = ["Labjack_channel1", "Labjack_channel2", "Labjack_channel3", "Labjack_channel4",
                         "Labjack_channel5",
    ...
    with open('./trainHistoryDict6On8ch', 'wb') as file_pi:
        pickle.dump(history.history, file_pi)

    model.save('./Amerilike_6ch_on_8ch.h5')
